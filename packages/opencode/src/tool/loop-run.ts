...
import { Loop } from "@opencode-ai/core/pipeline"
...
export const LoopRunTool = Tool.define<typeof Parameters, Metadata, never>(
  ...
  Effect.gen(function* () {
    return {
      description:
        "Use when a goal needs iterative improvement until convergence (e.g., fix flaky tests, optimize until a metric is met).",
      parameters: Parameters,
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context<Metadata>) =>
        Effect.gen(function* () {
          const maxIterations = params.maxIterations ?? 3
          const loop = new Loop.LoopEngine(filePath, { maxIterations })
