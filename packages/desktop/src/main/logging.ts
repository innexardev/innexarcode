...
export async function exportDebugLogs() {
  ...
  const output = join(app.getPath("downloads"), `innexarcode-debug-${stamp()}.zip`)
  try {
...
function serverLogRoots() {
  ...
  return [...new Set([join(xdgData, "innexarcode", "log"), join(app.getPath("userData"), "innexarcode", "log")])]
}
