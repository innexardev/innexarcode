...
- **WorkspaceIsolation (13.1)** — `packages/core/src/pipeline/workspace-isolation.ts`: cada agente paralelo trabalha em git worktree isolado (path determinístico por branch em tmpdir/wt), nunca no mesmo diretório que outro agente. Edits no worktree não afetam o repo principal. removeWorktree limpa worktree + branch após merge validado. Reuso de branch existente para retomada.
